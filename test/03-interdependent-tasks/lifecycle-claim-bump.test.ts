...
// `test/_setup/helpers/helpers.test.ts` so the registry instance read by the
// JobFactory mock is the same one `setMockJobsByType` writes to (different
// import paths can yield different module instances).
...
// `test/_setup/helpers/` resolves fixtures relative to its own folder, so we
// call WorkflowFactory directly here for a fixture co-located with this task.
