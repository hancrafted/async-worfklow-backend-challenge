...
- **Single-process Node, in-process worker pool.** Default `N=1`, override via `WORKER_POOL_SIZE`. All workers share one `AppDataSource` and run as coroutines on the same event loop. Default pinned at 1 because the shared `AppDataSource` (= one SQLite connection) cannot host concurrent `BEGIN`/`SAVEPOINT`/`COMMIT` boundaries from multiple coroutines (`SQLITE_ERROR: no such savepoint`); rationale + lift conditions in §Task 7 below.
...
- **`DEFAULT_WORKER_POOL_SIZE` dropped from `3` to `1`.** The production runtime (`src/index.ts`) shares one `AppDataSource` — and therefore one SQLite connection — across every coroutine spawned by `startWorkerPool(...)`. TypeORM's `manager.transaction(...)` opens `BEGIN` / `SAVEPOINT typeorm_N` / `COMMIT` boundaries on that single connection; concurrent coroutines cannot interleave them and trip `SQLITE_ERROR: no such savepoint: typeorm_1` / `cannot commit - no transaction is active` — the same substrate failure the integration tests work around with the `drainPool(...)` per-pool mutex (Task 3c Wave 3 caveat above) and Issue [#17](https://github.com/hancrafted/async-worfklow-backend-challenge/issues/17). Shipping `default=3` would mean the production worker pool can hit the *exact same* errors under genuine task concurrency — a latent crash surface the test suite cannot cover. The pragmatic option (this task, "option A" in the user-approved diagnosis) is to pin the default at 1 so the shipped configuration matches what the shared-connection substrate can host. Multi-worker concurrency stays available via `WORKER_POOL_SIZE=N` for reviewers who want to exercise the atomic-claim race manually — they accept the substrate caveat consciously, the same way the integration tests do.
  - *Production-grade:* per-worker `DataSource` instances backed by a real DB (Postgres, or SQLite WAL with separate per-worker connections). Issue #17 lands that change; once it does, this default lifts back to `N>1` (likely `3`, matching the original Task 3c Wave 3 rationale of "enough concurrency to demonstrate the atomic-claim race"). The same change retires the `drainPool(...)` mutex and decouples `EmailNotificationJob`'s 500ms latency cap from the test ceiling.
- **Why not flip to per-worker DataSources right now (option B)?** That is Issue #17, scoped as a Task 3c follow-up. It involves a non-trivial change to both the test helpers (`drainPool(...)`) and the boot path (`AppDataSource` becomes per-worker) and would balloon this commit well past the narrow "fix the shipped default" mandate. CLAUDE.md "Pragmatism vs Production grade" is explicit on this: scope the fix to the coding challenge surface, document the production-grade path. This entry, plus the lift condition tied to #17, satisfies the documentation rule.
- **Test surface unchanged.** The integration tests in `tests/03-interdependent-tasks/tasks-can-be-chained-through-dependencies.test.ts` that exercise the pool with `N=3` (`startWorkerPool({ size: 3, ... })`) and the unit-level race test (`tickOnce — error path: simulated race against a single queued task`) both pass an explicit `size`/iteration count; they do not depend on `DEFAULT_WORKER_POOL_SIZE` and continue to assert the atomic-claim primitive holds at N>1. The only test that asserts the default value (`resolveWorkerPoolSize — env validation`) is updated to `toBe(1)`.
