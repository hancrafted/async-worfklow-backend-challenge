...
- **Report job under fail-fast.** Under the workflow-aborts-on-first-failure semantic (Task 3), `ReportGenerationJob` only ever runs in fully-successful workflows. README §2's *"include error information for failed tasks"* requirement is therefore satisfied via `finalResult` (framework-owned), which always carries `{ status, error }` for failed tasks. The report job's body intentionally has no error-handling branch — it never sees a failed-dep envelope under fail-fast, and the runner's per-task try/catch covers the malformed-envelope case (e.g. corrupted upstream `Result.data` causing `buildDependencyEnvelope` to throw — see the integration error-path test).
...
- **`stepNumber` (not `taskId`) is the public identifier in the report.** Each `tasks[]` entry exposes `{ stepNumber, taskType, output }` only — the internal UUID `taskId` from the `JobDependencyOutput` envelope is stripped before serialization, per PRD §Implementation Decision 4 / US16. This is a deliberate divergence from the README's literal example shape (`taskId`), locked in the PRD: internal UUIDs never leak through any public surface, and `stepNumber` is the user-facing identifier callers already see in `/status` and `finalResult` (Task 4). The unit + integration tests both assert no `taskId` substring appears anywhere in the serialized report.
  - *Production-grade:* same — `stepNumber` is the documented public identifier across every public read surface.
- **`finalReport` is a framework-owned summary string with no per-job business logic.** Format: `"Generated report for workflow <id> with N tasks"`. Per the issue, this is *not* a hook for per-workflow narrative aggregation; it's a deterministic, machine-readable string that satisfies the README's `finalReport` field without inviting drift between job authors. Job-level business logic that needs richer text aggregation belongs in a downstream consumer of the report payload, not in the report job's own output.
  - *Production-grade:* the `finalReport` string moves to a dedicated reporting service; the report job stays a thin envelope projection.
- **`example_workflow.yml` deliberately NOT updated to use `reportGeneration`.** The shipped 6-step DAG (Task 3c-iii Wave 3) was tuned to exercise the full worker-pool surface: parallel roots of heterogeneous task types, fan-in / fan-out, serial chains. Adding a `reportGeneration` step would re-tune the DAG just to demonstrate the new job type — a circular concern, since the integration test (`tests/02-report-generation/report-generation.test.ts`) already drives `reportGeneration` end-to-end against the real `JobFactory` + real jobs. The `example_workflow.yml` boot fixture stays focused on the worker-pool surface; the manual test plan documents the temporary YAML edit needed to exercise `reportGeneration` via curl.
  - *Production-grade:* a workflow-template catalogue (PRD `POST /analysis` future-state) would carry `reportGeneration` examples in its own seed set; `example_workflow.yml` would no longer be the only public fixture.
