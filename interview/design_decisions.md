...
- **`Task.dependsOn` storage.** Stored as a `simple-json` array of UUIDs (resolved from YAML `stepNumber`s by the `WorkflowFactory` in a two-pass save). Single indexed query for readiness checks.
