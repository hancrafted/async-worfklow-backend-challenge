...
describe("Task 3b-ii Wave 2 — promotion + dependency envelope (integration)", () => {
  ...
  describe("error path", () => {
    it("when step 1 fails, step 2 stays waiting (no promotion on Failed) and the workflow ends Failed", async () => {
      // Promotion only fires on Completed transitions (PRD §Decision 9).
      // A failing step 1 must leave step 2 unpromoted; with no Wave 3 sweep
      // present, step 2 simply remains waiting and the workflow remains
      // non-Completed (in_progress on this Wave). The minimum guarantee Wave
      // 2 must hold is: step 2's job is never invoked.
      ...
      // because step 2 is still waiting (no promotion on Failed).
      ...
      // Wave 3 brings the fail-fast sweep that flips the workflow Failed
      // immediately; until then the workflow stays in_progress (allTerminal
      // is false because step 2 is still waiting).
      expect(workflowAfter.status).toBe(WorkflowStatus.InProgress);
    });
  });
});
